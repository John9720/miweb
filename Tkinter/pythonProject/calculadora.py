...
class Calculadora(tk.Tk):
    ...
    def _creacion_componentes(self):
        ...
        boton_limpiar = tk.Button(botones_frame, text="C", width=38, height=3, bd=0, bg="#eee", cursor="hand2",
                                  command=self._evento_limpiar)
        boton_limpiar.grid(row=0, column=0, columnspan=3)
        ...
        boton_restar = tk.Button(botones_frame, text="-", width=10, height=3, bd=0, bg="#eee", cursor="hand2",
                                      command=lambda: self._evento_click("-"))
        ...
        boton_sumar = tk.Button(botones_frame, text="+", width=10, height=3, bd=0, bg="#eee", cursor="hand2",
                                 command=lambda: self._evento_click("+"))
        boton_sumar.grid(row=3, column=3, padx=1, pady=1)
        boton_cero = tk.Button(botones_frame, text="0", width=24, height=3, bd=0, bg="#fff", cursor="hand2",
                               command=lambda: self._evento_click("0"))
        boton_cero.grid(row=4, column=0, columnspan=2, padx=1, pady=1)
        boton_decimal = tk.Button(botones_frame, text=".", width=10, height=3, bd=0, bg="#eee", cursor="hand2",
                                  command=lambda: self._evento_click("."))
        boton_decimal.grid(row=4, column=2, padx=1, pady=1)
        boton_resolver = tk.Button(botones_frame, text="=", width=10, height=3, bd=0, bg="#eee", cursor="hand2",
                                command=self._evento_resolver)
        boton_resolver.grid(row=4, column=3, padx=1, pady=1)
